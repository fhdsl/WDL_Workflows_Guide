...
-   `dockerSL` attribute allows users to use Docker and softlinks only to specific locations on the FH HPC Scratch disk. It is helpful when working with very large files. An example is: "ubuntu:latest". Just like the `docker` attribute, you should only specify this if you want the task to run in a container (otherwise the default will be a non-containerized job).
